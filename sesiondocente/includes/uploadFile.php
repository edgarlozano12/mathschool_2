...
$user_name=$_SESSION['docente']
 ...
 <?php
   ...
   $check_docu_quer="select * from docente WHERE usuario='$user_name'";
...
if($pasaImgSize == true)
{
	...
	if(in_array($tipo, $fileType) && $tamanio>0 && $tamanio<=maxUpload && ($extension[$num]=='jpg' || $extension[$num]=='png'))
	{
      ...
      $check_user="select * from docente WHERE usuario='$user_name'";
			...
			if(mysqli_num_rows($run_query)>0)
			{
					...
					$update_url="UPDATE `docente` SET `foto` = '$imgFile' WHERE `docente`.`usuario` = '$user_name'";
