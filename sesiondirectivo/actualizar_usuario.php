...
include("../database/db_conection.php");//make connection here
                    ...
                    if(isset($_POST['register']))
                    {
                        ...
                        $check_user_query="select * from directivos WHERE usuario='$user_name'";
                        $run_query=mysqli_query($dbcon,$check_user_query);

                        if(mysqli_num_rows($run_query)>0)
                        {
                            //update the pass into the database.
                            $update_usuario="UPDATE `directivos` SET `usuario` = '$user_usuario' WHERE `directivos`.`usuario` = '$user_name'";
