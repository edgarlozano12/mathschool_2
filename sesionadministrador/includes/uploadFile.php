...
$user_name=$_SESSION['admin']
 ...
 <?php
   ...
   $check_docu_quer="select * from administrador WHERE usuario='$user_name'";
...
if($pasaImgSize == true)
{
	...
	if(in_array($tipo, $fileType) && $tamanio>0 && $tamanio<=maxUpload && ($extension[$num]=='jpg' || $extension[$num]=='png'))
	{
      ...
      $check_user="select * from administrador WHERE usuario='$user_name'";
			...
			if(mysqli_num_rows($run_query)>0)
			{
					...
					$update_url="UPDATE `administrador` SET `foto` = '$imgFile' WHERE `administrador`.`usuario` = '$user_name'";
