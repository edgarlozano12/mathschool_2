...
if(!empty($_POST['captcha']))
{
                ...
                if($user_captcha==$_SESSION['img_number'])
                {
                    if(isset($_POST['register']))
                    {
                        ...
                        $check_user_query="select * from administrador WHERE clave='$user_pass0'";
                        $run_query=mysqli_query($dbcon,$check_user_query);

                        if(mysqli_num_rows($run_query)>0)
                        {
                            //update the pass into the database.
                            $update_pass="UPDATE `administrador` SET `clave` = '$user_pass1' WHERE `administrador`.`usuario` = '$user_name'";
